...
// AdmissionPolicy represents a parsed admission-style policy (variables + validations).
// Used for both VAP and MAP policies.
...
// EvalAdmission evaluates a full admission-style policy (preamble vars -> policy vars -> validations).
...
// ParseAdmissionPolicy parses an admission policy from YAML content.
func ParseAdmissionPolicy(yamlContent string) (*AdmissionPolicy, error) {
	return parseAdmissionPolicy([]byte(yamlContent))
}

// ParseAdmissionPolicyFile reads and parses an admission policy from a file.
